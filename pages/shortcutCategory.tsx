...
import { saveShortcutCategory } from "@/service/characterApi";

interface Props {}

export default function ShortcutCategory({}: Props) {
  ...
  async function onSaveHandler() {
    ...
    if (dataList?.length > 0) {
      for (const dataListElement of dataList) {
        const response = await saveShortcutCategory(dataListElement);
