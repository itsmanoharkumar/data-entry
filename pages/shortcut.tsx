...
import { saveShortcut } from "@/service/characterApi";
  ...
  async function onSaveHandler() {
    ...
    if (csvData && csvData.length > 0) {
      csvData.forEach((item: any) => {
        const data = {
          data: {
            shortText: item["name"],
            windowsKeyCombo: item.win,
            macKeyCombo: item.mac,
            shortcut_category: item.categoryId,
    ...
    if (dataList?.length > 0) {
      for (const dataListElement of dataList) {
        const response = await saveShortcut(dataListElement);
